...
from ..config import session,engine ,bcrypt
...
def get_machine_performance():
  ...
  for ele in machine:
      ...
      produce_qty = engine.execute(sql).fetchone()
      ...
      worktime = engine.execute(sql).fetchone()
      ...
      downtime = engine.execute(sql).fetchone()
